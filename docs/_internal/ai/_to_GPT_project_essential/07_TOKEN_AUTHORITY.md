...
- [Spacing Authority Contract](./SPACING_AUTHORITY.md) - LOCKED
- [Radius Authority Contract](./RADIUS_AUTHORITY.md) - LOCKED
- [Typography Authority Contract](./TYPOGRAPHY_AUTHORITY.md) - LOCKED
- [Motion Authority Contract](./MOTION_AUTHORITY.md) - LOCKED
- [Elevation Authority Contract](./ELEVATION_AUTHORITY.md) - LOCKED

**Source of Truth:** [FOUNDATION_LOCK.md](./FOUNDATION_LOCK.md) - Foundation lock status
...
Foundation tokens are semantically neutral and can be used across all components. They are defined in `src/FOUNDATION/tokens/`:

- **`spacing`** - Spacing utilities (px-*, py-*, gap-*, etc.)
- **`typography`** - Font sizes, weights, line heights, letter spacing
- **`colors`** - Color system with CSS variables
- **`gradients`** - Reusable gradient patterns (brand, surface, overlay, text)
...
Gradient tokens provide reusable gradient patterns that use token-based colors. They are defined in `src/FOUNDATION/tokens/gradients.ts`:
...
// Use gradient tokens instead of inline gradient classes
<div className={GRADIENT_TOKENS.brand.primary} />
<div className={GRADIENT_TOKENS.text.brand} />
```

**Rule:** All gradients must use `GRADIENT_TOKENS`. Inline gradient classes are not allowed.

#### Semantic Color Tokens

Semantic color tokens provide meaning-based colors for UI feedback and states. They are defined in `src/FOUNDATION/tokens/colors.ts`:

- **`success`** - Success/positive feedback (green)
- **`error`** - Error/negative feedback (red)
- **`warning`** - Warning/caution feedback (orange)
- **`info`** - Informational feedback (blue)

Each semantic color has a foreground variant for text:
- **`success-foreground`** - Text color for success backgrounds
- **`error-foreground`** - Text color for error backgrounds
- **`warning-foreground`** - Text color for warning backgrounds
- **`info-foreground`** - Text color for info backgrounds
...
const successColor = semanticColors[mode].success;
```

**Rule:** Use semantic tokens for feedback states (success, error, warning, info). Use brand colors (primary, accent, secondary) for brand identity elements.

**Note:** The `destructive` color is an alias for `error` and should be used for destructive actions (delete, remove, etc.).
...
- **`TEXT_TOKENS`** - Typography utilities (fontSize, fontWeight, lineHeight, letterSpacing)
  ...
  - **Reference:** [Text / Typography Lock](./locks/TEXT_LOCK.md)
...
**Note:** For enforcement mechanisms, see enforcement documents in the Foundation Document Classification table in [FOUNDATION_LOCK.md](./FOUNDATION_LOCK.md).
...
- [Spacing Authority Contract](./SPACING_AUTHORITY.md) - Spacing token authority (LOCKED)
- [Radius Authority Contract](./RADIUS_AUTHORITY.md) - Radius token authority (LOCKED)
- [Typography Authority Contract](./TYPOGRAPHY_AUTHORITY.md) - Typography token authority (LOCKED)
- [Motion Authority Contract](./MOTION_AUTHORITY.md) - Motion token authority (LOCKED)
- [Elevation Authority Contract](./ELEVATION_AUTHORITY.md) - Elevation token authority (LOCKED)
- [TUI Token System Audit Report](../../../docs_archive/reports/archive/archive/reports/other/TOKEN_SYSTEM_AUDIT.md) - Detailed audit findings (archived)
