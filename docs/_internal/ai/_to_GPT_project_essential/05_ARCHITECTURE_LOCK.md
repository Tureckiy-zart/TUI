...
**Foundation Component Lock Sweep:** Foundation component lock status was verified and confirmed on 2026-01-02 (Task TUI_FOUNDATION_LOCK_SWEEP_01). All 19 Foundation components (16 LOCKED + 3 DISCREPANCY resolved) have been verified against architecture rules, Public API canon, token authority, and pipeline requirements. See `docs/reports/foundation/TUI_FOUNDATION_LOCK_SWEEP_01_STATUS.md` and `docs/reports/foundation/TUI_FOUNDATION_LOCK_SWEEP_01_VIOLATIONS.md` for detailed verification results.

**Foundation Component Lock Sweep Finalization:** Foundation Component Lock Sweep was formally finalized on 2026-01-02 (Task TUI_FOUNDATION_LOCK_SWEEP_FINAL_01). All 21 Foundation components have been confirmed as locked, compliant, and immutable. Foundation layer is complete, compliant, and officially finalized. See `docs/architecture/FOUNDATION_LOCK.md` (section "Foundation Component Lock Sweep Finalization") for the complete list of finalized components and verification results.
...
**Note:** Foundation is CLOSED; specific components may be temporarily unlocked as LEGACY UNLOCKED strictly for canonical migration. Canonical truth: FOUNDATION_LOCK.md. **Foundation Component Lock Sweep (2026-01-02):** All Foundation components verified and confirmed. See FOUNDATION_LOCK.md for complete list.
...
| **Dropdown** | `src/COMPOSITION/overlays/Dropdown/` | ✅ **PROCESS LOCKED** | 2026-01-02 | `docs/reports/audit/DROPDOWN_BASELINE_REPORT.md` |
| **Spacer** | `src/COMPOSITION/layout/Spacer/` | ✅ **PROCESS LOCKED** | 2026-01-02 | `docs/reports/audit/SPACER_BASELINE_REPORT.md` |
...
| **Navbar** | `src/COMPOSITION/layout/Navbar/` | ✅ **PROCESS LOCKED** | 2026-01-01 | `docs/reports/audit/NAVBAR_BASELINE_REPORT.md` |
| **Panel** | `src/COMPOSITION/layout/Panel/` | ✅ **PROCESS LOCKED** | 2026-01-02 | `docs/reports/audit/PANEL_BASELINE_REPORT.md` |
| **ButtonGroup** | `src/COMPOSITION/actions/ButtonGroup/` | ✅ **PROCESS LOCKED** | 2026-01-02 | `docs/reports/audit/BUTTONGROUP_BASELINE_REPORT.md` |
...
| **Menu** | `src/COMPOSITION/navigation/Menu/` | ✅ **PROCESS LOCKED** | 2026-01-02 | `docs/reports/audit/MENU_BASELINE_REPORT.md` |
...
## 🔒 File Structure Canon Lock (LOCKED)

**Status:** ✅ **LOCKED**  
**Lock Date:** 2026-01-02  
**Priority:** CRITICAL  
**Source of Truth:** [`docs/architecture/ARCHITECTURE_STATE.md`](./ARCHITECTURE_STATE.md)

The **File Structure Canon** is **LOCKED** and **IMMUTABLE**. The current directory structure is declared **CANONICAL** and **MUST NOT** change without explicit unlock procedure.

### What Is Locked

**Canonical Root Directories (LOCKED):**

1. **`src/FOUNDATION/`** - Foundation layer (tokens and theme system)
   - **Status:** ✅ **LOCKED** - Immutable
   - **Purpose:** Tokens only for active components
   - **Rule:** FOUNDATION tokens **MUST NOT** exist without active components

2. **`src/PRIMITIVES/`** - Primitives layer
   - **Status:** ✅ **LOCKED** - Canonical structure
...
3. **`src/COMPOSITION/`** - Composition layer
   - **Status:** ✅ **LOCKED** - Canonical structure
   - **Purpose:** Layout, overlays, interaction orchestration
   - **Sub-layers:**
     - `COMPOSITION/overlays/` - All overlay components (LOCKED)
     - `COMPOSITION/layout/` - Layout components (LOCKED)
     - `COMPOSITION/navigation/` - Navigation components (LOCKED)
     - `COMPOSITION/controls/` - Control components (LOCKED)
   - **Rule:** All overlays **MUST** live in COMPOSITION layer only

4. **`src/PATTERNS/`** - Patterns layer
   - **Status:** ✅ **LOCKED** - Canonical structure
   - **Purpose:** Business/UI patterns (no overlays)
   - **Rule:** PATTERNS **MUST NOT** define overlay primitives or overlay infrastructure

5. **`src/DOMAIN/`** - Domain layer
   - **Status:** ✅ **LOCKED** - Canonical structure
   - **Purpose:** App-specific sections

### Lock Rules

**FORBIDDEN Actions (without unlock):**

1. ❌ **No new top-level `src/` directories** - Cannot create new root directories without unlock procedure
2. ❌ **No cross-layer file relocation** - Cannot move files between layers without unlock procedure
3. ❌ **No documentation contradicting ARCHITECTURE_STATE.md** - All documentation must align with canonical structure
4. ❌ **No structural changes without unlock** - Any structural modification requires explicit unlock procedure

**REQUIRED Compliance:**

1. ✅ **ARCHITECTURE_STATE.md is the sole source of truth** - All structure references must align with ARCHITECTURE_STATE.md
2. ✅ **Documentation consistency** - All architecture documents must reference correct canonical paths
3. ✅ **Layer boundaries enforced** - Components must be placed in correct layers per ARCHITECTURE_STATE.md
4. ✅ **No orphan files** - All files must be properly organized within canonical structure

### File Structure Unlock Procedure

Any structural changes require explicit unlock procedure:

**Step 1: Explicit Unlock Request**
- Create explicit task with justification
- Document why the change is necessary
- Identify all affected directories and files

**Step 2: Full Structure Audit**
- Complete audit of all affected directories
...
**Step 5: Re-lock**
- Update ARCHITECTURE_STATE.md with new canonical state
- Update ARCHITECTURE_CONTEXT.md if needed
...
**Violation Handling:**
- Any structural change without unlock is considered an **architectural violation**
- Violations must be **refused immediately** with reference to this lock
- Violations must be **documented** and **reverted** if discovered
...
- ✅ Readiness status confirmed: READY FOR STRUCTURE LOCK

**Reference:** [TUI File Structure Audit Report](../../reports/audit/TUI_FILE_STRUCTURE_AUDIT_REPORT.md)
...
- **v1.8** (2026-01-02): VisuallyHidden Component Pipeline 18A Complete
  - Added VisuallyHidden to Extension Layer (a11y utilities)
  - VisuallyHidden status: PROCESS LOCKED (Extension component lock)
  - Pipeline 18A completed (Steps 0-12)
  - Completion Date: 2026-01-02
  - Audit Report: `docs/reports/audit/VISUALLYHIDDEN_BASELINE_REPORT.md`
  - Architectural decisions:
    - Component type: Utility component (non-interactive a11y wrapper)
    - Inline styles acceptable: Component uses inline styles with structural CSS properties (position absolute, 1px size, clip-path inset) for visually-hidden pattern
    - Rationale: Visually-hidden pattern requires specific structural CSS values (1px size, -1 margin) that cannot be tokenized
    - Decision: Acceptable exception for structural CSS properties in visually-hidden pattern (no visual tokens needed)
    - No size/variant props: Component is intentionally single-purpose utility (correct design)
    - No CVA structure: Component does not use CVA (Decision Matrix RULE 2 - no token-driven axes, uses inline styles instead)
    - Non-interactive wrapper: Component does not handle user interactions, does not interfere with parent accessibility
  - Quality: High (comprehensive test suite, A11Y tests added, Storybook compliant)
  - Completed per Pipeline 18A

- **v1.9** (2026-01-02): FocusTrap Component Pipeline 18A Complete
  - Added FocusTrap to Extension Layer (focus utilities)
  - FocusTrap status: PROCESS LOCKED (Extension component lock)
  - Pipeline 18A completed (Steps 0-12)
  - Completion Date: 2026-01-02
  - Audit Report: `docs/reports/audit/FOCUSTRAP_BASELINE_REPORT.md`
  - Architectural decisions:
    - Component type: Utility component (non-visual focus management wrapper)
    - No visual tokens: Component is intentionally non-visual (invisible wrapper, no visual styling)
    - NO MOTION BY DESIGN: Component uses programmatic focus management (no visual animations)
    - No size/variant props: Component is utility, not visual component (correct design)
    - No CVA structure: Component does not use CVA (Decision Matrix RULE 2 - no token-driven axes, non-visual component)
    - Focus management: Component traps Tab/Shift+Tab within children, manages initial focus, restores focus on unmount
    - Keyboard parity: Tab/Shift+Tab for focus navigation (keyboard-only interaction, no pointer equivalent needed)
    - Escape semantics: Optional callback, no-op if not provided (matches INPUT_AUTHORITY.md)
    - State blocking: active=false disables focus trap (matches INPUT_AUTHORITY.md)
    - Focus trap compliance: Matches FOCUS_AUTHORITY.md requirements (F-TRAP-1, F-TRAP-3, F-RESTORE-1, F-RESTORE-2, F-TAB-1)
    - A11Y compliance: Component is invisible wrapper, no accessible name/ARIA roles required
  - Quality: High (comprehensive test suite, A11Y tests, Focus tests, Input tests, Storybook compliant)
  - Completed per Pipeline 18A

- **v1.11** (2026-01-02): ButtonGroup Action Composition Component Pipeline 18A Complete
  - Added ButtonGroup to Extension Layer (actions category)
  - ButtonGroup status: PROCESS LOCKED (Extension component)
  - Pipeline 18A completed (Steps 0-12)
  - Completion Date: 2026-01-02
  - Audit Report: `docs/reports/audit/BUTTONGROUP_BASELINE_REPORT.md`
  - Architectural decisions:
    - Component type: Extension Action Composition Component (groups multiple Button components)
    - Composition pattern: Uses Stack for layout, Context for prop propagation
    - No CVA usage: Composition component using Stack (Decision Matrix RULE 2 - no token-driven axes, correct)
    - Size/variant propagation: Props propagated via Context (size, variant, disabled)
    - No own variants: ButtonGroup does not have own size/variant variants (correct - propagates to Button)
    - Token compliance: Token-based spacing only (SpacingValue type)
    - Accessibility: role="group", aria-orientation for vertical (correct ARIA usage)
    - Non-interactive: Delegates all interaction to Button components (correct responsibility boundary)
    - Story requirements: Default story added (STORYBOOK_STORIES_STANDARD compliance)
    - Matrix/States/SizesGallery: NOT REQUIRED (size/variant are propagated, not own variants)
  - Quality: High (comprehensive test coverage, Storybook compliant, A11Y compliant)
  - Completed per Pipeline 18A
...
**Status:** ✅ **LOCKED**  
**Lock Date:** 2026-01-02  
**Authority:** This lock is binding and immutable without explicit unlock procedure
...
**Forbidden:**
- ❌ Deep imports from internal paths
- ❌ Direct imports from barrel files
...
- ✅ All Foundation components listed in `FOUNDATION_LOCK.md` are exported
- ✅ No Foundation component duplicates allowed
- ✅ No alternative Foundation implementations allowed

**Extension Components:**
- ✅ Extension components **MAY be exported** if listed as ALLOWED in `EXTENSION_STATE.md`
- ❌ RESTRICTED components **MUST NOT** be exported
- ❌ Internal-only components **MUST NOT** be exported

**Design Tokens:**
- ✅ All design tokens are exported (~180+ exports)
- ✅ Token types and values are exported
- ✅ Token utility functions are exported

**Forbidden Exports:**
- ❌ Radix UI primitives (direct exports)
- ❌ Internal implementation details
- ❌ Legacy components
- ❌ Duplicate Foundation components
- ❌ RESTRICTED Extension components

### Type System Rules

**Required:**
- ✅ Explicit union types for all variant/size props
- ✅ No CVA-derived types in public API (`VariantProps<typeof cvaVariants>`)
- ✅ No `string` widening for variant/size props
- ✅ No `any` types in public API

**CVA Variants:**
- ⚠️ CVA variants **MAY be exported** ONLY if required for Extension composition
- ⚠️ Each exported variant must be verified for Extension composition needs
...
**Required:**
- ✅ Explicit exports only: `export { Button, type ButtonProps } from "./PRIMITIVES/Button"`
- ❌ Wildcard exports: `export * from "./PRIMITIVES/Button"`
...
- **Foundation Lock Sweep Finalization:** `docs/architecture/FOUNDATION_LOCK.md` (Task TUI_FOUNDATION_LOCK_SWEEP_FINAL_01, finalized 2026-01-02)
