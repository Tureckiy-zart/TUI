/**
 * ESLint Rule: no-classname-on-foundation
 *
 * Disallow passing className to Foundation components in consumer code.
 *
 * This rule enforces the Foundation Contract which prohibits className
 * prop injection into Foundation components at the consumer level.
 *
 * @enforcement TUNG: TUI_CSV2_ESLINT_GUARDS_CONSUMER_013
 * @rule className prop is forbidden on Foundation components in consumer code
 */
...
type MessageIds = "noClassNameOnFoundation";

export const noClassNameOnFoundation = ESLintUtils.RuleCreator(
  () => "docs/architecture/closed-system/CLOSED_SYSTEM_V2_ENFORCEMENT_GUARDS.md",
)<[], MessageIds>({
  name: "no-classname-on-foundation",
  meta: {
    type: "problem",
    docs: {
      description: "Disallow passing className to Foundation components in consumer code",
    },
    messages: {
      noClassNameOnFoundation:
        "Passing className to Foundation components is forbidden by the Foundation Contract. Use component props for styling instead.",
    },
    schema: [],
    fixable: undefined,
  },
